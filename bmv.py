...
def main():
    ...
    parser.add_argument("-t", "--type", type=str, help="Specify genre", default=nonfiction_keywords[0], choices=(fiction_keywords + nonfiction_keywords + textbook_keywords))
    parser.add_argument("-m", "--move", action="store_true", help="Move the ePub file to the new location (by default only the new filename is printed)")
    parser.add_argument("--force", action="store_true", help="Overwrite the target file if it already exists. Useless without -m/--move.")
